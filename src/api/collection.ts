...
export const getCollectionList = async (): Promise<CollectionListItemDto[] | null> => {
  try {
    const response = await authenticatedFetch(env.API_URL + '/collection/list')
...
export const getCollectionByCollectionID = async (collection_id: string): Promise<CollectionDetailDto | null> => {
  try {
    const response = await authenticatedFetch(`${env.API_URL}/collection/${collection_id}`)
...
export const startQuiz = async (collection_id: string, user_id: string) => {
  const response = await authenticatedFetch(`${env.API_URL}/quiz/start`, {
    method: 'POST',
...
export const submitQuizAnswer = async (payload: SubmitQuizAnswerRequest) => {
  const response = await authenticatedFetch(`${env.API_URL}/quiz/answer`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export const endQuiz = async (payload: EndQuizRequest) => {
  const response = await authenticatedFetch(`${env.API_URL}/quiz/end`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  })
